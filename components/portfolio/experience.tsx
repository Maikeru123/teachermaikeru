      ...
      <div className="overflow-hidden bg-zinc-800 px-5 py-9 text-white sm:px-6 sm:py-10 dark:bg-zinc-900">
        <div>
          <div className="flex items-start justify-between gap-4">
            <h2 id="experience-heading" className="text-[clamp(1.65rem,3vw,2.3rem)] font-medium tracking-[-0.07em]">PROFESSIONAL EXPERIENCE</h2>
        ...
        <div className="relative mt-7 border-y border-white/15 py-5 sm:py-6">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div><p className="text-[15px] text-zinc-300">University of Mindanao</p><h3 className="mt-1 text-[22px] font-medium tracking-[-0.04em]">IT Educator</h3></div>
            ...
            {responsibilityGroups.map((group) => (
              <section key={group.title}>
                ...
                <div className="mt-2 space-y-2">
                  {group.items.map((item) => <p className="flex gap-2 text-[13px] leading-relaxed text-zinc-300" key={item}><span className="mt-[0.45rem] size-1 shrink-0 rounded-full bg-white" />{item}</p>)}
                </div>
              </section>
            ))}
          </div>
        ...
        <article className="border-b border-white/15 py-5 sm:py-6">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div><p className="text-[15px] text-zinc-300">Aquilla Software</p><h3 className="mt-1 text-[22px] font-medium tracking-[-0.04em]">Software Development Intern</h3></div>
            <span className="rounded-full border border-white/20 px-3 py-1.5 text-[11px] text-zinc-200">3-Month Internship</span>
          </div>
          <p className="mt-5 max-w-[900px] text-[13px] leading-relaxed text-zinc-300 sm:text-[14px]">Completed a three-month software development internship at Aquilla Software, where I contributed to a Doctor-Patient Management System. The experience provided hands-on exposure to developing a real-world software solution and working within a professional development environment.</p>
          <div className="mt-5 rounded-sm border border-white/15 bg-white/[0.035] p-4 sm:p-5">
            ...
            <h4 className="mt-2 text-[17px] font-medium">Doctor-Patient Management System</h4>
            <p className="mt-1 text-[13px] leading-relaxed text-zinc-300">A management system designed to support and organize doctor-patient processes and information within a centralized application.</p>
          </div>
          <div className="mt-5 grid gap-x-10 gap-y-2 md:grid-cols-2">
            ...
            ].map((highlight) => <p className="flex gap-2 text-[13px] leading-relaxed text-zinc-300" key={highlight}><span className="mt-[0.45rem] size-1 shrink-0 rounded-full bg-white" />{highlight}</p>)}
          </div>
        </article>
      </div>
    </section>
